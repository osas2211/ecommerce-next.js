...
export default function ProductCard({img, title, rating, progress, amount, id, count, category}) {
  ...
  return (
    <div className={styles.product_ + " rounded-3"}>
      ...
      <div className={styles.product_details}>
        <Link href={`/collections/${category}/${title}`}><a className={styles.title}>{title}</a></Link>
      ...
      <Link href={`/collections/${category}/${title}`}><a className='btn-custom-2 mt-2 w-100 text-center'>view details</a></Link>
    </div>
  )
}
